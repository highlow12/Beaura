...
function worker(options: { rejectRuntimeWrite?: boolean } = {}) {
  ...
  let online=true, activated=false, largestAddAllBatch=0;
  ...
  const contentFiles=['/generated/manifest.json','/generated/catalog.json',...Array.from({length:25},(_,index)=>`/generated/questions/test-${index}.json`)];
  let source=readFileSync('src/service-worker.ts','utf8').replace(/import[^;]+from "\$service-worker";/,`const build=["/_app/start.js"];const files=${JSON.stringify(contentFiles)};const version="test";`);
  ...
  return {lifecycle,request,setOffline:()=>{online=false;},activated:()=>activated,largestAddAllBatch:()=>largestAddAllBatch};
}
describe('offline installed app',()=>{
  it('preloads generated content in bounded batches and serves unseen routes offline',async()=>{
    ...
    expect(await (await sw.request('/generated/questions/test-24.json'))?.text()).toBe('https://app.test/generated/questions/test-24.json');
  });
