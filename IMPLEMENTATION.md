...
- 15개 활성 트랙, 236개 레슨, 1,190개 문제와 트랙 간 선행 조건을 포함한 DAG.
- 9종 입력 Renderer와 공통 Question Host, 첫 오답 재시도, 최종 결과 저장.
- Dexie 원자적 학습 이벤트·상태·Outbox 저장, 중복 방지와 세션 복구.
- ts-fsrs 5.4.2 복습 일정, 경험치·연속 학습·일일 목표.
- 하트 3개·8시간 회복·로컬 자정 초기화 정책과 데이터 호환성은 유지하되, 베타 테스트를 위해 차감과 전역 UI는 임시 비활성화.
...
- 콘텐츠 fetch 실패 메시지에 요청 경로와 원인을 포함하고, 화면에서 재시도할 수 있는 오류를 표시. 서비스 워커가 감지한 same-origin 네트워크 실패는 로컬 진단과 동의 기반 Sentry에도 기록.
...
HTTP 오류 응답과 JSON 파싱 실패처럼 서비스 워커의 네트워크 실패 감지에 해당하지 않는 로딩 오류는 화면에 표시되지만 자동 진단이나 Sentry에 별도로 전달되지 않는다.
