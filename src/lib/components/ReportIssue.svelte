...
{#if open}
  <div class="backdrop" role="presentation" onclick={(event) => { if (event.target === event.currentTarget) open = false; }}>
    <section class="report-dialog card stack" role="dialog" aria-modal="true" aria-labelledby="report-title">
      ...
      <p class="muted">무슨 일이 있었는지 적으면 GitHub 신고 화면을 엽니다. 최근 자동 진단 정보 {reportCount}건 중 최대 5건이 함께 첨부됩니다.</p>
      <label>
        문제 설명
        <textarea bind:value={description} rows="5" maxlength="2000" placeholder="예: 복습 문제를 제출했는데 다음 화면으로 넘어가지 않았습니다."></textarea>
      </label>
      <p class="privacy-note">진단 정보에는 오류 메시지, 발생 시각, 현재 화면 경로, 앱 빌드 정보만 포함합니다. 학습 답안과 학습 기록은 넣지 않습니다.</p>
      <div class="actions">
