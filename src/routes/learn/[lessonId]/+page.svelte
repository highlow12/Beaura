<script lang="ts">
  ...
  import { advanceLesson, recordAnswer, recordDelayedRetry, previousContentIndex, type LessonSession } from '$lib/lesson/lesson-engine';
  ...
  // Reading prior explanations is a view-only detour, never persisted as lesson progress.
  ...
  function restoreStepState(nextSession: LessonSession, nextLesson: Lesson) {
    ...
    const activeFlow = nextSession.currentIndex >= nextLesson.flow.length
      ? nextLesson.flow.find((step) => step.type === 'question' && step.ref === nextSession.retryQueue?.[nextSession.retryCursor ?? 0])
      : nextFlow;
    resumedAnswer = activeFlow?.type === 'question' && (nextSession.currentIndex >= nextLesson.flow.length
      ? (nextSession.retryResults ?? []).includes(activeFlow.ref)
      : nextSession.answers.some((answer) => answer.questionId === activeFlow.ref));
    ready = resumedAnswer;
    reviewIndex = null;
  }
  ...
  $effect(() => {
    ...
    navigationEpoch++;
    let cancelled = false;
    loading = true; error = ''; saveError = ''; ready = false; resumedAnswer = false; saving = false; reviewIndex = null; lesson = null; session = null; blocked = []; loadedCurriculum = null; lessonStates = []; unlockedLessons = [];
    void (async () => {
      try {
  ...
  async function next() {
    if (reviewIndex !== null) { reviewIndex = null; return; }
  ...
  function previous() {
    if (saving || previousExplanation < 0) return;
    ...
    <section class="card learning-card">
      ...
      {:else if question}
        {#key `${lesson.id}:${session.currentIndex}:${session.retryCursor ?? 0}:${question.revision}`}
          <QuestionRenderer {question} singleAttempt paused={reviewIndex !== null} onCompleted={completeQuestion} onReady={() => { ready = true; }} />
        {/key}
      {/if}
      </div>
