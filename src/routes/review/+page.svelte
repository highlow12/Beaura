...
<div class="stack review-shell">
  ...
  {:else if !started}
    ...
    {#if data.queue.length}<button class="button" disabled={starting} onclick={start}>{starting ? '복습 준비 중…' : '복습 시작'}</button>{:else}<div class="actions"><a class="button" href={`${base}/learn`}>새로운 개념 배우기</a><button class="button secondary" onclick={refresh}>복습 다시 확인</button></div>{/if}
    </section>
