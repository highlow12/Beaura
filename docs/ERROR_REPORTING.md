...
콘텐츠 JSON 네트워크 요청이 실패하면 repository가 요청 경로와 브라우저가 제공한 원인 메시지를 포함한 오류를 발생시킨다. HTTP 오류에는 상태 코드가 포함된다. 각 화면은 이 오류를 처리해 사용자에게 표시하고 가능한 경우 재시도 동작을 제공한다.

배포 앱에서 서비스 워커가 감지한 same-origin 네트워크 실패는 화면의 예외 처리와 별개로 로컬 자동 진단에 기록되며, 동의한 사용자에 한해 Sentry에도 전송된다. HTTP 오류 응답이나 JSON 파싱 실패처럼 서비스 워커의 네트워크 실패 감지에 해당하지 않는 오류는 현재 화면에만 표시된다.
