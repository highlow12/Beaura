...
오류 자동 보고는 선택적 동의 기반으로만 동작한다. 동의 전에는 원격 오류 SDK를 초기화하거나 진단 정보를 전송하지 않으며, 거부·철회해도 핵심 학습 기능에 불이익이 없다. 동의 상태는 계정·기기 식별자 없이 로컬에 저장하고, 전송 이벤트는 사용자 식별자·쿠키·답안·학습 기록·상세 요청 데이터를 제외한다. 구체적인 수집 항목과 국외 이전은 개인정보 처리방침과 `docs/ERROR_REPORTING.md`를 따른다.
